...
Sources read per project:
  ...
  *.testql.toon.yaml      → root-level and nested toon scenario files
  ...
  project/*.toon.yaml     → code2llm/redup/vallm analysis (health, evolution, map, duplication)
  project/*.mmd           → Mermaid diagrams (call graph, flow)
  project/context.md      → architecture context generated by code2llm
...
# ---------------------------------------------------------------------------
# Project analysis files (generated by code2llm, redup, vallm, etc.)
# ---------------------------------------------------------------------------

# ---------------------------------------------------------------------------
# map.toon.yaml generator
# ---------------------------------------------------------------------------
...
def generate_map_toon(proj_dir: Path) -> str:
    ...
    Produces the same toon format as code2llm map output:
      - Header with file/line/language stats
      - M[N]: module list with line counts
      - D: per-module export + function signature details
    """
    ...
    # ── Python-only deep analysis ───────────────────────────────────────────
    py_modules: list[tuple[Path, dict]] = []
    all_funcs: list[dict] = []  # {name, cc, fan, mod}
    ...
    # Merge: build alert list (alternating CC and fan-out, dedup)
    ...
    # Language summary: sorted by count descending
...
def generate_sumd_content(  # noqa: C901
    ...
    if project_analysis:
        sources_used.append(f"project/({len(project_analysis)} analysis files)")
    ...
    # ── Code Analysis (project/ dir — code2llm, redup, vallm output) ────
    if project_analysis:
        a("## Code Analysis")
        a("")
